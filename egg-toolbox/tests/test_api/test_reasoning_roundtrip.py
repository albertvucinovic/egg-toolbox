...
# =================================================================== #
# Qwen3 history-thinking preservation                                 #
# =================================================================== #

# Excerpt of Qwen3's actual baked template's assistant branch -- the
# piece that gates <think>...</think> preservation on
# ``loop.index0 > ns.last_query_index``.  We rebuild just enough of
# the template to exercise the patcher without pulling in a GGUF.
...
class TestQwen3HistoryThinkingPreservation:
    def _messages(self):
        return [
    ...
    def test_default_strips_history_thinking(self):
        """Without the opt-in, Qwen3's gate fires and past <think>
        is stripped -- this is what happens today on the user's setup."""
        t = ChatTemplate(_QWEN3_LIKE_TEMPLATE)
        rendered = t.render(self._messages(), add_generation_prompt=True)
        # The assistant message's <think> should NOT appear in
        # rendered output because it's not the last message.
        assistant_section = rendered[
            rendered.find("<|im_start|>assistant"):
            rendered.find("<|im_end|>", rendered.find("<|im_start|>assistant"))
        ]
        assert "<think>" not in assistant_section
        assert "Okay, let me think" not in assistant_section
        assert "Once upon a time." in rendered

    def test_opt_in_preserves_history_thinking(self):
        """With preserve_history_thinking=True, <think> survives on
        non-last assistant messages."""
        t = ChatTemplate(_QWEN3_LIKE_TEMPLATE, preserve_history_thinking=True)
        rendered = t.render(self._messages(), add_generation_prompt=True)
        assert "<think>\nOkay, let me think\n</think>" in rendered
        assert "Once upon a time." in rendered

    def test_env_var_honored(self, monkeypatch):
        """EGG_PRESERVE_HISTORY_THINKING=1 acts as the default."""
        monkeypatch.setenv("EGG_PRESERVE_HISTORY_THINKING", "1")
        t = ChatTemplate(_QWEN3_LIKE_TEMPLATE)
        rendered = t.render(self._messages(), add_generation_prompt=True)
        assert "<think>\nOkay, let me think\n</think>" in rendered

    def test_explicit_false_overrides_env(self, monkeypatch):
        """An explicit False beats the env var."""
        monkeypatch.setenv("EGG_PRESERVE_HISTORY_THINKING", "1")
        t = ChatTemplate(_QWEN3_LIKE_TEMPLATE, preserve_history_thinking=False)
        rendered = t.render(self._messages(), add_generation_prompt=True)
        # History thinking NOT preserved since we explicitly said False.
        assert "Okay, let me think" not in rendered

    def test_noop_on_non_qwen_template(self):
        """Templates without the Qwen3 gate pattern are unchanged."""
        simple = "{%- for m in messages -%}{{ m.role }}: {{ m.content }}\n{% endfor -%}"
        t = ChatTemplate(simple, preserve_history_thinking=True)
        # No exception, no substitution.  Source should equal input.
        assert t.source == simple
