...
from .anthropic import messages as anthropic_messages
...
def create_app(orchestrator: Orchestrator) -> Starlette:
    ...
    async def handle_anthropic_messages(request: Request) -> JSONResponse:
        try:
            raw = await request.body()
            body = json.loads(raw)
        except (json.JSONDecodeError, ValueError) as e:
        ...
        if not isinstance(body["messages"], list) or len(body["messages"]) == 0:
            return _error_response(
                ...
                "invalid_request_error", "invalid_field",
            )
        if "max_tokens" not in body:
            return _error_response(
                400, "Missing required field: 'max_tokens'",
                "invalid_request_error", "missing_field",
            )
        try:
            return await anthropic_messages(body, orchestrator)
        except Exception:
            traceback.print_exc()
            return _error_response(
